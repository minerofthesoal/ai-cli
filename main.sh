...
<title>AI CLI Chat</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:sans-serif;background:#1e1e2e;color:#cdd6f4;height:100vh;display:flex;flex-direction:column}
h3{padding:10px 16px;background:#181825;border-bottom:1px solid #313244;color:#89b4fa;flex-shrink:0}
#msgs{flex:1;overflow-y:auto;padding:16px;display:flex;flex-direction:column;gap:8px}
.msg-user{background:#313244;padding:10px 14px;border-radius:8px;margin-left:40px;white-space:pre-wrap}
.msg-ai{background:#181825;border:1px solid #313244;padding:10px 14px;border-radius:8px;margin-right:40px;white-space:pre-wrap}
.msg-err{background:#45161a;border:1px solid #f38ba8;padding:10px 14px;border-radius:8px;color:#f38ba8}
#bar{background:#181825;padding:10px;display:flex;gap:8px;border-top:1px solid #313244;flex-shrink:0}
#inp{flex:1;background:#313244;border:1px solid #45475a;color:#cdd6f4;padding:10px;border-radius:6px;font-size:14px;resize:none;font-family:inherit}
#inp:focus{outline:none;border-color:#89b4fa}
#send-btn{background:#89b4fa;color:#1e1e2e;border:none;padding:10px 18px;border-radius:6px;cursor:pointer;font-weight:700;font-size:14px}
...
<div id="msgs"></div>
<div id="bar">
  <textarea id="inp" rows="2" placeholder="Message... (Enter to send, Shift+Enter for newline)"></textarea>
  <button id="send-btn">Send</button>
</div>
<script>
(function() {
  ...
  async function send() {
    ...
    try {
      ...
      addMsg('msg-err', 'Error: ' + e.message);
    }
  }
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>AI CLI Dashboard</title>
<style>
*, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
body   { font-family: system-ui, sans-serif; background: #1e1e2e; color: #cdd6f4; min-height: 100vh; }
nav    { background: #181825; padding: 10px 20px; display: flex; gap: 10px; border-bottom: 2px solid #313244; align-items: center; flex-wrap: wrap; }
nav h2 { color: #89b4fa; font-size: 1.1rem; margin-right: auto; white-space: nowrap; }
.tab-btn { background: #313244; color: #cdd6f4; border: none; padding: 8px 18px; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 600; transition: background 0.15s; }
...
h3     { color: #89b4fa; margin-bottom: 14px; font-size: 1.1rem; }
h4     { color: #a6e3a1; margin: 16px 0 8px; font-size: 0.95rem; }
.out   { background: #181825; border: 1px solid #313244; border-radius: 8px; padding: 14px; margin: 10px 0; white-space: pre-wrap; max-height: 420px; overflow-y: auto; font-family: monospace; font-size: 13px; line-height: 1.5; min-height: 60px; color: #a6e3a1; }
.out.idle { color: #6c7086; font-style: italic; }
textarea, input[type="text"], input[type="password"] { background: #313244; border: 1px solid #45475a; color: #cdd6f4; padding: 10px; border-radius: 6px; font-family: monospace; font-size: 14px; width: 100%; }
textarea:focus, input:focus { outline: none; border-color: #89b4fa; }
.btn  { background: #89b4fa; color: #1e1e2e; border: none; padding: 9px 20px; border-radius: 6px; cursor: pointer; font-weight: 700; margin: 4px; font-size: 13px; transition: background 0.15s; }
...
.btn-row { display: flex; flex-wrap: wrap; gap: 4px; margin-bottom: 12px; }
...
.row textarea { flex: 1; height: 48px; resize: none; }
/* Chat */
#chat-msgs { background: #181825; border: 1px solid #313244; border-radius: 8px; padding: 12px; height: 360px; overflow-y: auto; display: flex; flex-direction: column; gap: 8px; margin-bottom: 10px; }
.cm-user { background: #313244; padding: 8px 12px; border-radius: 8px; margin-left: 60px; white-space: pre-wrap; }
.cm-ai   { background: #11111b; border: 1px solid #313244; padding: 8px 12px; border-radius: 8px; margin-right: 60px; white-space: pre-wrap; }
.cm-err  { background: #45161a; border: 1px solid #f38ba8; padding: 8px 12px; border-radius: 8px; color: #f38ba8; }
/* Terminal */
#term-out { background: #0d0d0d; border: 1px solid #313244; border-radius: 8px; padding: 12px; height: 320px; overflow-y: auto; font-family: monospace; font-size: 13px; color: #00ff41; white-space: pre-wrap; }
#term-auth { text-align: center; padding: 48px 20px; }
#term-auth p { color: #6c7086; margin-bottom: 16px; }
#term-auth input { max-width: 220px; text-align: center; margin-bottom: 12px; }
</style>
</head>
<body>
<nav>
  <h2>AI CLI Dashboard</h2>
...
<div id="tab-chat" class="panel active">
  ...
  <div class="row">
    <textarea id="chat-inp" placeholder="Message... (Enter=send, Shift+Enter=newline)"></textarea>
...
<div id="tab-models" class="panel">
  ...
  <div class="btn-row">
    ...
    <button class="btn btn-sm" id="models-recommended">Recommended</button>
    <button class="btn btn-sm" id="models-status">Status</button>
  </div>
  <div id="models-out" class="out idle">Click Refresh to load models...</div>
  <h4>Download Model</h4>
  <div class="row">
    <input type="text" id="models-hf" placeholder="HuggingFace model ID (e.g. TheBloke/Mistral-7B-v0.1-GGUF)">
    <button class="btn btn-sm" id="models-dl">Download</button>
  </div>
  <div id="models-dl-out" class="out idle" style="margin-top:8px">Download output will appear here...</div>
</div>
...
<div id="tab-settings" class="panel">
  ...
  <div class="btn-row">
    <button class="btn btn-sm" id="set-health">Health</button>
    <button class="btn btn-sm" id="set-backends">Backends</button>
    <button class="btn btn-sm" id="set-stats">Stats</button>
    <button class="btn btn-sm" id="set-config">Config</button>
  </div>
  <div id="settings-out" class="out idle">Click a button to view status...</div>
</div>
...
<div id="tab-tools" class="panel">
  <h3>Quick Tools</h3>
  <div class="grid">
    <button class="btn btn-sm" data-cmd="bench">Benchmark</button>
    <button class="btn btn-sm" data-cmd="error-codes">Error Codes</button>
    <button class="btn btn-sm" data-cmd="alias list">Aliases</button>
    <button class="btn btn-sm" data-cmd="status">Full Status</button>
    <button class="btn btn-sm" data-cmd="memory list">Memory</button>
    <button class="btn btn-sm" data-cmd="extension locate">Extensions</button>
  </div>
  <div id="tools-out" class="out idle">Click a tool to run it...</div>
</div>

<!-- Terminal -->
<div id="tab-terminal" class="panel">
  <h3>Terminal (Password Protected)</h3>
  <div id="term-auth">
    <p>Enter the terminal password to unlock</p>
    <input type="password" id="term-pw" placeholder="Password" maxlength="64">
    <br>
    <button class="btn" id="term-unlock">Unlock</button>
  </div>
  <div id="term-shell" style="display:none">
    <div id="term-out"></div>
    <div class="row" style="margin-top:8px">
      <span style="color:#00ff41;padding:10px;font-family:monospace;flex-shrink:0">$</span>
      <input type="text" id="term-cmd" placeholder="Type a command and press Enter...">
      <button class="btn btn-sm" id="term-run">Run</button>
    </div>
  </div>
</div>
...
(function() {
  ...
  async function runCmd(out, cmd) {
    setOut(out, 'Running...', false);
    try {
      var data = await apiPost('/v3/cmd', {cmd: cmd});
      setOut(out, data.output || '(no output)', false);
    } catch(e) {
      setOut(out, 'Error: ' + e.message, false);
    }
  }
  ...
  async function chatSubmit() {
    ...
    chatAdd('cm-user', text);
    chatHist.push({role: 'user', content: text});
    try {
      var data = await apiPost('/v1/chat/completions', {messages: chatHist});
      ...
      chatAdd('cm-err', 'Error: ' + e.message);
    }
  ...
  /* ---- models ---- */
  var modelsOut   = document.getElementById('models-out');
  var modelsDlOut = document.getElementById('models-dl-out');

  document.getElementById('models-refresh').addEventListener('click', async function() {
    setOut(modelsOut, 'Loading...', false);
    try {
      var data = await apiGet('/v2/models');
      var lines = (data.data || []).map(function(m) {
        return (m.id || '?') + (m.size ? '  [' + m.size + ']' : '') + (m.type ? '  (' + m.type + ')' : '');
      });
      setOut(modelsOut, lines.length ? lines.join('\n') : 'No models found.', false);
    } catch(e) {
      setOut(modelsOut, 'Error: ' + e.message, false);
    }
  });

  document.getElementById('models-recommended').addEventListener('click', function() {
    runCmd(modelsOut, 'recommended');
  });

  document.getElementById('models-status').addEventListener('click', function() {
    runCmd(modelsOut, 'status');
  });

  document.getElementById('models-dl').addEventListener('click', function() {
    var hfid = document.getElementById('models-hf').value.trim();
    if (!hfid) { setOut(modelsDlOut, 'Please enter a HuggingFace model ID.', true); return; }
    runCmd(modelsDlOut, 'download ' + hfid);
  });

  /* ---- settings ---- */
  var settingsOut = document.getElementById('settings-out');

  document.getElementById('set-health').addEventListener('click', async function() {
    setOut(settingsOut, 'Loading...', false);
    try {
      var data = await apiGet('/health');
      setOut(settingsOut, JSON.stringify(data, null, 2), false);
    } catch(e) {
      setOut(settingsOut, 'Error: ' + e.message, false);
    }
  });

  document.getElementById('set-backends').addEventListener('click', async function() {
    setOut(settingsOut, 'Loading...', false);
    try {
      var data = await apiGet('/v2/backends');
      var lines = (data.backends || []).map(function(b) {
        return b.id + '  [' + b.type + ']  ' + (b.available ? 'available' : 'unavailable');
      });
      lines.push('', 'Active: ' + (data.active || 'auto'));
      setOut(settingsOut, lines.join('\n'), false);
    } catch(e) {
      setOut(settingsOut, 'Error: ' + e.message, false);
    }
  });

  document.getElementById('set-stats').addEventListener('click', async function() {
    setOut(settingsOut, 'Loading...', false);
    try {
      var data = await apiGet('/v2/stats');
      setOut(settingsOut, JSON.stringify(data, null, 2), false);
    } catch(e) {
      setOut(settingsOut, 'Error: ' + e.message, false);
    }
  ...
  document.getElementById('set-config').addEventListener('click', function() {
    runCmd(settingsOut, 'config');
  });

  /* ---- tools ---- */
  var toolsOut = document.getElementById('tools-out');

  document.querySelectorAll('[data-cmd]').forEach(function(btn) {
    btn.addEventListener('click', function() {
      runCmd(toolsOut, btn.getAttribute('data-cmd'));
    });
  });

  /* ---- terminal ---- */
  var termPw     = document.getElementById('term-pw');
  var termUnlock = document.getElementById('term-unlock');
  var termAuth   = document.getElementById('term-auth');
  var termShell  = document.getElementById('term-shell');
  ...
  async function termLogin() {
    var pw = termPw.value;
    try {
      var data = await apiPost('/v3/terminal/auth', {password: pw});
      if (data.ok) {
        termAuthed = true;
        savedPw = pw;
        termAuth.style.display = 'none';
        termShell.style.display = 'block';
        termAppend('Terminal unlocked. Type commands below.');
        termCmd.focus();
      } else {
  ...
  async function termExec() {
    if (!termAuthed) return;
    var cmd = termCmd.value.trim();
    if (!cmd) return;
    termCmd.value = '';
    termAppend('$ ' + cmd);
    try {
      var data = await apiPost('/v3/terminal/exec', {cmd: cmd, password: savedPw});
      termAppend(data.output || data.error || 'Done');
    } catch(e) {
      termAppend('Error: ' + e.message);
    }
  }

  termUnlock.addEventListener('click', termLogin);
  termPw.addEventListener('keydown', function(e) {
    if (e.key === 'Enter') termLogin();
  });
  termRun.addEventListener('click', termExec);
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        elif self.path == "/v3/cmd":
            # Safe, read-only ai sub-commands for the dashboard buttons
            cmd_str = b.get("cmd", "").strip()
            ALLOWED = {
                "models", "recommended", "status", "bench", "config",
                "error-codes", "alias list", "memory list",
                "extension locate", "download"
            }
            base = cmd_str.split()[0] if cmd_str else ""
            ok = (cmd_str in ALLOWED) or any(cmd_str.startswith(a) for a in ALLOWED)
            if not ok:
                self._j(403, {"error": "Command not allowed: " + cmd_str})
            else:
                parts = cmd_str.split()
                try:
                    import subprocess as _sp
                    r = _sp.run([CLI] + parts, capture_output=True, text=True, timeout=30)
                    out = (r.stdout + r.stderr).strip() or "(no output)"
                except Exception as ex:
                    out = "Error: " + str(ex)
                self._j(200, {"output": out, "cmd": cmd_str})
