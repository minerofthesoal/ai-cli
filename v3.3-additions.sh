...
# Auto-detect and use LM Studio models folder if available
_ LMSTUDIO_DIR="$(detect_lmstudio_dir)"
_ OLLAMA_STATUS="$(detect_ollama)"
_ GPT4ALL_DIR="$(detect_gpt4all_dir)"
_ TEXTGEN_DIR="$(detect_textgen_dir)"
...
_ai_complete() {
  ...
  local cmds="ask a chat code review explain summarize translate pipe
    ...
    profile completion setup status bench health
    api rlhf ttm mtm Mtm dataset extension
    plugin test change changelog -aup -Su -L -h -gui -gui+ -C -Cf"
  ...
  elif [[ "$prev" == "ai" && "$cur" == -* ]]; then
    COMPREPLY=($(compgen -W "-h --help -v --version -aup -Su -L -gui -gui+ -C -Cf" -- "$cur"))
...
cmd_model_info() {
  ...
  if [[ -n "$_LMSTUDIO_DIR" ]]; then
    echo -e "  LM Studio:        ${BGREEN}detected${_R} at $_LMSTUDIO_DIR"
    echo "  LM Studio GGUFs:  $(find "$_LMSTUDIO_DIR" -maxdepth 3 -name '*.gguf' 2>/dev/null | wc -l)"
  else
    echo "  LM Studio:        not detected"
  fi
  if [[ -n "$_OLLAMA_STATUS" ]]; then
    echo -e "  Ollama:           ${BGREEN}detected${_R} ($_OLLAMA_STATUS)"
  else
    echo "  Ollama:           not detected"
  fi
...
cmd_recommend_quant() {
  ...
  if command -v nvidia-smi &>/dev/null; then
    ...
    [[ -z "$vram_gb" ]] && vram_gb=16  # Apple Silicon default estimate
  fi
  ...
  # Build table based on available VRAM
  local _v; _v="${vram_gb}"
  if [[ $_v -ge 80 ]]; then
